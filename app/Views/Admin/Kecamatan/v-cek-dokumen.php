...
<div class="container mt-4">
    ...
    <!-- Tombol Validasi Berkas & Cetak -->
    <form action="<?= base_url('kecamatan-validasi-berkas') ?>" method="post" target="_blank">
        <input type="hidden" name="id_permohonan" value="<?= $permohonan['id_permohonan'] ?>">
        <button type="submit" class="btn btn-primary">Validasi Berkas & Cetak</button>
    </form>

    <!-- Tombol Simpan Surat Setelah Preview -->
    <form action="<?= base_url('kecamatan-simpan-surat') ?>" method="post">
        <button type="submit" class="btn btn-success mt-2">Simpan Surat</button>
    </form>
